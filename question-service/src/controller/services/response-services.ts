...
export function createNoContentResponse(response: Response, message?: string) {
  return response
    .status(204)
    .json({ data: { message: message ?? "No content to return." } });
}

export function createNotFoundResponse(response: Response, message?: string) {
  return response
    .status(404)
