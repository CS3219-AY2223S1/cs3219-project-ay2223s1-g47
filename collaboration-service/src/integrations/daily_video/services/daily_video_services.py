...
class DailyVideoService:
    ...
    def __init__(self,  _daily_server_host: str = DAILY_VIDEO_DOMAIN):
    ...
    async def create_video_room(self, unique_room_name: str = None, privacy: str = "public") -> DailyVideoRoom:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(self.daily_server_host + "/rooms", data=data, headers=self.headers) as response:
    ...
    async def delete_video_room(self, unique_room_name: str):
        ...
        # api call
        async with aiohttp.ClientSession() as session:
            async with session.delete(self.daily_server_host + "/rooms/" + unique_room_name, headers=self.headers) as response:
                response: aiohttp.ClientResponse
    ...
    async def get_video_room_by_id(self, unique_room_name: str) -> DailyVideoRoom:
        ...
        """
        async with aiohttp.ClientSession() as session:
            async with session.get(self.daily_server_host + "/rooms/" + unique_room_name, headers=self.headers) as response:
                response: aiohttp.ClientResponse
                response_json = await response.json()
                if response.status != 200:
                    raise VideoServiceException("Error creating video room!", detail=response_json)
